<template>
  <div class="sidebar container" :class="isOpened ? 'open' : ''" :style="cssVars">
...
export default {
  ...
  mounted() {
    ...
    mq.addEventListener('change', this.handleScreen)
    this.handleScreenChange(mq)
  },
  computed: {
    cssVars() {
      return {
  ...
  beforeDestroy() {
    const mediaQuery = window.matchMedia('(max-width: 520px)', '(min-width: 520px)')
    mediaQuery.removeEventListener(this.handleScreenChange, this.handleScreen)
  },
  watch: {
    isOpened() {
