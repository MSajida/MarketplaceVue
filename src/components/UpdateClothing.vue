                        ...
                        <b-card id="cardLogin" style="background-color: aliceblue;">
                            <b-form v-if="show" id="formRegister">
                                ...
                                <b-form-group id="input-group-1" label-for="noOFDaysUsed" label="No of Days Used"
                                    style="color: gray">
                                    <b-form-input id="noOFDaysUsed" class="input" v-model="Clothing.daysUsed" type="number" min="0"
                                ...
                                <b-form-group id="input-group-1" label-for="price" label="Price*" style="color: gray">
                                    <b-form-input id="price" class="input" v-model="Clothing.price" type="number" min="0"
...
export default ({
    ...
    computed:
    {
       isDisabled()
       {
        return this.Clothing.subcategory ===''|| this.Clothing.productName ==='' || this.Clothing.price==='' || this.Clothing.qtyAvailable==='';
       }
    },
    methods: {
        onFileSelected(event) {
            ...
            this.$axios
            .post("http://52.22.24.58:8082/clothing/updateProductImages/"+this.Clothing.id, this.formdata)
                .then((res) => {
                    if (res.status == 200) {
                        this.data = res;
